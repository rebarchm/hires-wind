...
# db_bsb <- '/home/ubuntu/hd2/src/hires-wind/birch.sqlite'
...
birch_plot_ids<-c("BC-1", "BC-10", "BC-100", "BC-101", "BC-102", "BC-103", "BC-106", "BC-11", "BC-12", "13", 
                  "BC-15", "BC-17", "BC-2", "BC-20", "BC-21", "BC-22", "BC-23", "BC-25", "BC-26", "BC-27", 
                  "BC-28", "BC-3", "BC-30", "BC-31", "BC-33", "BC-34", "BC-35", "BC-37", "BC-38", "BC-39",
                  "BC-4", "BC-40", "BC-42", "BC-44", "BC-45", "BC-46", "BC-48", "BC-49", "BC-50", "BC-54",
                  "BC-6", "BC-60", "BC-62", "BC-64", "BC-67", "BC-68", "BC-69", "BC-70", "BC-71", "BC-72", 
                  "BC-73", "BC-75", "BC-76", "BC-77", "BC-8", "BC-81", "BC-83", "BC-84", "BC-85", "BC-86", 
                  "BC-87", "BC-88", "BC-89", "BC-9", "BC-90", "BC-91", "BC-92", "BC-95", "BC-96", "BC-97", 
                  "BC-98", "BC-61", "BC-82", "BC-93")
...
shinyServer(function(input, output, session) {
  ...
  output$siteMap <- renderLeaflet({
    ...
    if (input$site == 'SRC') {
      loc <- dbFetchSensorLocation2(db_src, src_plot_ids)
      ...
      loc <- dbFetchSensorLocation2(db_bsb, bsb_plot_ids)
      lat <- mean(loc$Latitude)
      lon <- mean(loc$Longitude)
      row <- loc[loc$Plot_id == input$variable, ]
      zoom = 11
      ...
      loc <- dbFetchSensorLocation2(db_birch, birch_plot_ids)
      lat <- mean(loc$Latitude)
      lon <- mean(loc$Longitude)
      row <- loc[loc$Plot_id == input$variable, ]
      zoom = 10
  ...
  observe({
    ...
    if (input$site == 'SRC') {
      loc <- dbFetchSensorLocation2(db_src, src_plot_ids) #2
    }
    else if (input$site == 'BSB'){
      loc <- dbFetchSensorLocation2(db_bsb, bsb_plot_ids)
    }
  ...
  output$vectorMap <- renderPlot({
    ...
    plot <- makeVectorMap2(s, lat, lon, 12, min_time=input$timeRange[1], max_time=input$timeRange[2], hourly_averaging = input$hourly_avg)
  ...
  output$speedPlot <- renderPlot({
    ...
    if (input$dataSorting == 'Raw') {
      if(input$site == 'SRC'){
        ...
        end_date <- max(input$daterange)
        for (i in 1:length(input$multi_var)) {
          s<-dbFetchSensor2(db_src, input$multi_var[i], start_date, end_date)
          if (i == 1) {
            master <- s
          }
          else {
        ...
        end_date<-max(input$daterange)
        for (i in 1:length(input$multi_var)) {
          s<-dbFetchSensor2(db_bsb, input$multi_var[i], start_date, end_date)
        ...
        end_date <- max(input$daterange)
        for (i in 1:length(input$multi_var)) {
          s<-dbFetchSensor2(db_birch, input$multi_var[i], start_date, end_date)
          if (i == 1) {
            master <- s
          }
          else {
      ...
      if(input$site == 'SRC'){
        ...
        end_date<-max(input$daterange)
        for (i in 1:length(input$multi_var)) {
            s <- dbFetchHourlyAvg2(db_src, input$multi_var[i], start_date, end_date, avg_time=input$avgPer, align=input$avgJust)
        ...
        end_date<-max(input$daterange)
        for (i in 1:length(input$multi_var)) {
          s <- dbFetchHourlyAvg2(db_bsb, input$multi_var[i], start_date, end_date, avg_time=input$avgPer, align=input$avgJust)
          s <- data.frame(date_time=index(s), wind_speed = coredata(s))
          s$plot_id <- rep(input$multi_var[i], NROW(s))
          if (i == 1) {
            master <- s
        ...
        end_date<-max(input$daterange)
        for (i in 1:length(input$multi_var)) {
          s <- dbFetchHourlyAvg2(db_birch, input$multi_var[i], start_date, end_date, avg_time=input$avgPer, align=input$avgJust)
          s <- data.frame(date_time=index(s), wind_speed = coredata(s))
          s$plot_id <- rep(input$multi_var[i], NROW(s))
          if (i == 1) {
            master <- s
  ...
  output$dirPlot <- renderPlot({
    if (is.null(input$multi_var)) {
      ...
      if(input$site == 'SRC'){
        ...
        end_time<-max(input$daterange)
        for (i in 1:length(input$multi_var)) {
          s<-dbFetchSensor2(db_src, input$multi_var[i], start_time, end_time)
          if (i == 1) {
            master <- s
          }
          else {
        ...
        end_time<-max(input$daterange)
        for (i in 1:length(input$multi_var)) {
          s<-dbFetchSensor2(db_bsb, input$multi_var[i], start_time, end_time)
          colnames(s) <- c("plot_id", "date_time", "wind_speed", "wind_gust", "wind_dir", "quality", "sensor_quality")
          if (i == 1) {
            master <- s
          }
        ...
        end_time<-max(input$daterange)
        for (i in 1:length(input$multi_var)) {
           s<-dbFetchSensor2(db_birch, input$multi_var[i], start_time, end_time)
           if (i == 1) {
             master <- s
           }
           else {
  ...
  output$sensor_summary <- renderTable({
    ...
    if (input$site == 'SRC'){
      s <- dbFetchWind(db_src, input$variable, min(input$daterange), max(input$daterange), progressBar = TRUE)
    }
    else if (input$site == 'BSB'){
      s <- dbFetchWind(db_bsb, input$variable, min(input$daterange), max(input$daterange), progressBar = TRUE)
    }
    else if (input$site == 'BIRCH'){
      s <- dbFetchWind(db_birch, input$variable, min(input$daterange), max(input$daterange), progressBar = TRUE)
    }
  ...
  output$sensor_hist <- renderPlot({
    ...
    if (input$summary_data_type == "wind_speed") {
      ...
      xlab <- "Direction (0 = North, 90 = West, 180 = South, 270 = East)"   # Not sure if this is correct...
    }
  ...
  output$site_summary <- renderTable({ 
    ...
    if (input$site == 'SRC'){
      s<-dbFetchWind(db_src, src_plot_ids, min(input$daterange), max(input$daterange), progressBar = TRUE)
    }
    else if (input$site == 'BSB'){
      s<-dbFetchWind(db_bsb, bsb_plot_ids, min(input$daterange), max(input$daterange), progressBar = TRUE)
    }
    else if (input$site == 'BIRCH'){
      s<-dbFetchWind(db_birch, birch_plot_ids, min(input$daterange), max(input$daterange), progressBar = TRUE)
    }
  ...
  output$site_hist <- renderPlot({
    ...
    if (input$summary_data_type == "wind_speed") {
      ...
      hist_data <- s$obs_dir
      title <- "Wind Direction"
      xlab <- "Direction (0 = North, 90 = West, 180 = South, 270 = East)"
    }
  ...
  multiSensorDataset <- reactive({
    if(input$site == 'SRC'){
      for (i in 1: length(input$multi_var)) {
          s<-dbFetchWind(db_src, input$multi_var[i], min(input$daterange), max(input$daterange))
          if (i == 1 ) {
            data <- s
          }
          else {
      ...
      for (i in 1: length(input$multi_var)) {
        s<-dbFetchWind(db_bsb, input$multi_var[i], min(input$daterange), max(input$daterange))
        if (i == 1 ) {
          data <- s
        }
        else {
      ...
      for (i in 1: length(input$multi_var)) {
        s<-dbFetchWind(db_birch, input$multi_var[i], min(input$daterange), max(input$daterange))
        if (i == 1 ) {
          data <- s
        }
        else {
  ...
  sensorDataset <- reactive({
    if(input$site == 'SRC'){
        s<-dbFetchWind(db_src, input$variable, min(input$daterange), max(input$daterange))
    }
    else if(input$site == 'BSB'){
        s<-dbFetchWind(db_bsb, input$variable, min(input$daterange), max(input$daterange))
    }
    else if(input$site == 'BIRCH'){
        s<-dbFetchWind(db_birch, input$variable, min(input$daterange), max(input$daterange))
    }
    return(s)
  })
  ...
  siteDataset <- reactive({
    if(input$site == 'SRC'){
      s<-dbFetchWind(db_src, src_plot_ids, min(input$daterange), max(input$daterange))
    }
    else if(input$site == 'BSB'){
      s<-dbFetchWind(db_bsb, bsb_plot_ids, min(input$daterange), max(input$daterange))
    }
    else if(input$site == 'BIRCH'){
      s<-dbFetchWind(db_birch, birch_plot_ids, min(input$daterange), max(input$daterange))
    }
  })
